...
class CategoryDataTable extends DataTable
{
    ...
    {
        return (new EloquentDataTable($query->select('categories.*')))
            ->addIndexColumn()
            ...
            ->addColumn('action', function ($record) {
                ...
                if (Gate::check('category_edit')) {
                    $actionHtml .= '<a href="' . route('admin.categories.edit', $record->uuid) . '" class="btn btn-outline-primary btn-sm btnEditCategory" id="btnEditCategory" title="Edit"> <i class="ri-pencil-line"></i> </a>';
                }

                if (Gate::check('category_delete')) {
                    $actionHtml .= '<a href="javascript:void(0);" data-href="' . route('admin.categories.destroy', $record->uuid) . '" class="btn btn-outline-danger btn-sm btnDeleteCategory" title="Delete"> <i class="ri-delete-bin-line"></i> </a>';
                }
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('title')->title(trans('cruds.category.fields.title'))->addClass('dt-title')->searchable(true);
        $columns[] = Column::make('description')->title(trans('cruds.category.fields.description'))->addClass('dt-description')->searchable(true);
