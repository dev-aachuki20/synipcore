...
class ComplaintDataTable extends DataTable
{
    ...
    {
        return (new EloquentDataTable($query->with(['user.society'])->select('complaints.*')))
            ->addIndexColumn()
            ...
            ->editColumn('society', function ($record) {
                return $record->user ? ucfirst($record->user->society->name) : '';
            })
            ...
            ->editColumn('status', function ($record) {
                $options = config('constant.status_type.complaint_status');
                ...
                foreach ($options as $value => $label) {
                    ...
                    $html .= '<option value="' . $value . '"' . $isSelected . ' data-complaint_id="' . $record->uuid . '" data-complaint_status="' . $value . '">' . $label . '</option>';
                }
            ...
            ->addColumn('action', function ($record) {
                ...
                if (Gate::check('complaint_view')) {
                    $actionHtml .= '<a href="javascript:void(0);" data-href="' . route('admin.complaints.show', $record->uuid) . '" class="btn btn-outline-info btn-sm btnViewComplaint" title="Show"> <i class="ri-eye-line"></i> </a>';
                }

                if (Gate::check('complaint_delete')) {
                    $actionHtml .= '<a href="javascript:void(0);" data-href="' . route('admin.complaints.destroy', $record->uuid) . '" class="btn btn-outline-danger btn-sm btnDeleteComplaint" title="Delete"> <i class="ri-delete-bin-line"></i> </a>';
                }
            ...
            ->filterColumn('society', function ($query, $keyword) {
     ...
     */
    public function query(Complaint $model, Request $request): QueryBuilder
    {
        $user = auth()->user();
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('description')->title(trans('cruds.complaint.fields.message'))->addClass('dt-description')->searchable(true);
        $columns[] = Column::computed('society')->title(trans('cruds.complaint.fields.society'))->addClass('dt-society')->searchable(true);
        $columns[] = Column::make('user.name')->title(trans('cruds.complaint.fields.user'))->addClass('dt-user')->searchable(true);
        $columns[] = Column::make('status')->title(trans('cruds.complaint.fields.status'))->addClass('dt-status')->searchable(true);
        $columns[] = Column::make('created_at')->title(trans('cruds.complaint.fields.created_at'))->addClass('dt-created_at');
