...
class FaqDataTable extends DataTable
{
    ...
    {
        return (new EloquentDataTable($query->select('faqs.*')))
            ->addIndexColumn()
            ...
            ->editColumn('short_description', function ($record) {
                return '<div class="two-line-dt-description">' . ($record->short_description ?? '') . '</div>';
            })

            ->addColumn('action', function ($record) {
                $actionHtml = '';

                if (Gate::check('faq_edit')) {
                    $actionHtml .= '<a href="' . route('admin.faqs.edit', $record->uuid) . '" class="btn btn-outline-primary btn-sm btnEditFaq" id="btnEditFaq" title="Edit"> <i class="ri-pencil-line"></i> </a>';
                }

                if (Gate::check('faq_delete')) {
                    $actionHtml .= '<a href="javascript:void(0);" data-href="' . route('admin.faqs.destroy', $record->uuid) . '" class="btn btn-outline-danger btn-sm btnDeleteFaq" title="Delete"> <i class="ri-delete-bin-line"></i> </a>';
                }
     ...
     */
    public function query(Faq $model, Request $request): QueryBuilder
    {
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('title')->title(trans('cruds.faq.fields.title'))->addClass('dt-title')->searchable(true);
        $columns[] = Column::make('short_description')->title(trans('cruds.faq.fields.short_description'))->addClass('dt-description')->searchable(true);
