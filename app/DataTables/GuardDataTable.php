...
class GuardDataTable extends DataTable
{
    ...
    {
        return (new EloquentDataTable($query->with(['society', 'building'])->select('users.*')))
            ->addIndexColumn()
            ...
            ->editColumn('society.name', function ($record) {
                return $record->society ? ucfirst($record->society->name) : null;
            })

            ->editColumn('building.title', function ($record) {
                return $record->building ? ucfirst($record->building->title) : null;
            })

            ->addColumn('action', function ($record) {
                $actionHtml = '';
                /* if (Gate::check('guard_view')) {
                    $actionHtml .= '<a href="javascript:void(0);" data-href="' . route('admin.guards.show', $record->uuid) . '" class="btn btn-outline-info btn-sm btnViewGuard" title="Show"> <i class="ri-eye-line"></i> </a>';
                } */

                if (Gate::check('guard_edit')) {
                    $actionHtml .= '<a href="' . route('admin.guards.edit', $record->uuid) . '" data-href="' . route('admin.guards.edit', $record->uuid) . '" class="btn btn-outline-success btn-sm btnEditGuard" title="Edit"> <i class="ri-pencil-line"></i> </a>';
                }

                if (Gate::check('guard_delete')) {
                    $actionHtml .= '<a href="javascript:void(0);" data-href="' . route('admin.guards.destroy', $record->uuid) . '" class="btn btn-outline-danger btn-sm btnDeleteGuard" title="Delete"> <i class="ri-delete-bin-line"></i> </a>';
                }
            ...
            ->filterColumn('building.title', function ($query, $keyword) {
                $query->whereHas('building', function ($q) use ($keyword) {
                    $q->where('buildings.title', 'like', "%$keyword%");
                });
            })
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('name')->title(trans('cruds.guard.fields.name'))->addClass('dt-name')->searchable(true);
        $columns[] = Column::make('society.name')->title(trans('cruds.guard.fields.society'))->addClass('dt-society')->searchable(true);
        $columns[] = Column::make('building.title')->title(trans('cruds.resident.fields.building'))->addClass('dt-building')->searchable(true);
        $columns[] = Column::make('created_at')->title(trans('cruds.guard.fields.created_at'))->addClass('dt-created_at');
