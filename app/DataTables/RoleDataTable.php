...
class RoleDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($record) {
                ...
                if (Gate::check('role_edit')) {
                    $actionHtml .= '<a href="' . route('admin.roles.edit', $record->id) . '" class="btn btn-outline-primary btn-sm btnEditRole" id="btnEditRole" title="Edit"> <i class="ri-pencil-line"></i> </a>';
                }

                if (Gate::check('role_delete')) {
                    $actionHtml .= '<a href="javascript:void(0);" class="btn btn-outline-danger btn-sm deleteRoleBtn" data-href="' . route('admin.roles.destroy', $record->id) . '" title="Delete"><i class="ri-delete-bin-line"></i></a>';
                }
                return $actionHtml;
            })
            ->rawColumns(['action']);
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('name')->title(trans('cruds.role.fields.role_name'));
