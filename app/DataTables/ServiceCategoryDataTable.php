...
class ServiceCategoryDataTable extends DataTable
{
    ...
    {
        return (new EloquentDataTable($query->with(['user'])->select('service_categories.*')))
            ->addIndexColumn()
            ...
            ->editColumn('status', function ($record) {
                return isset($record) && !empty($record->status) ? ucfirst($record->status) : '';
            })

            ->addColumn('action', function ($record) {
                $actionHtml = '';

                if (Gate::check('service_category_edit')) {
                    $actionHtml .= '<a href="' . route('admin.service-categories.edit', $record->uuid) . '" class="btn btn-outline-primary btn-sm btnEditServiceCategory" id="btnEditServiceCategory" title="Edit"> <i class="ri-pencil-line"></i> </a>';
                }

                if (Gate::check('service_category_delete')) {
                    $actionHtml .= '<a href="javascript:void(0);" data-href="' . route('admin.service-categories.destroy', $record->uuid) . '" class="btn btn-outline-danger btn-sm btnDeleteServiceCategory" title="Delete"> <i class="ri-delete-bin-line"></i> </a>';
                }
     ...
     */
    public function query(ServiceCategory $model, Request $request): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('service-category-table')
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('title')->title(trans('cruds.service_category.fields.title'))->addClass('dt-title')->searchable(true);
        $columns[] = Column::make('status')->title(trans('cruds.service_category.fields.status'));
        // $columns[] = Column::make('created_at')->title(trans('cruds.service_category.fields.created_at'))->addClass('dt-created_at')->searchable(true);
