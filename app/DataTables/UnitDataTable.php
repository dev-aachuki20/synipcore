...
class UnitDataTable extends DataTable
{
    ...
    {
        return (new EloquentDataTable($query->with(['society', 'building'])->select('units.*')))
            ->addIndexColumn()
            ...
            ->editColumn('society.name', function ($record) {
                return ucfirst($record->society->name ?? null);
            })
     ...
     */
    public function query(Unit $model, Request $request): QueryBuilder
    {
        $user = auth()->user();

        if ($user->is_sub_admin) {
            return $model->newQuery()->where('units.society_id', $user->society_id);
        } else {
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('title')->title(trans('cruds.unit.fields.title'))->addClass('dt-title')->searchable(true);
        $columns[] = Column::make('society.name')->title(trans('cruds.unit.fields.society'))->addClass('dt-building')->searchable(true);
        $columns[] = Column::make('building.title')->title(trans('cruds.unit.fields.building'))->addClass('dt-building')->searchable(true);
