<script>
    $(document).ready(function() {
        $(document).on("submit", "#cameraAddForm, #cameraEditForm", function(e) {
            e.preventDefault();
            var url = $(this).data('url');
            var formData = new FormData(this);
            storeUpdateCamera(formData, url)
        });
    });
    ...
    function storeUpdateCamera(formData, url) {
        $('.loader-div').show();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = "{{ route('admin.cameras.index') }}";
                    }, 1000);
                } else {
