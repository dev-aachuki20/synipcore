...
<script>
    $(document).ready(function() {
        ...
        $(document).on("submit", "#maintenanceItemAddForm, #maintenanceItemEditForm", function(e) {
            e.preventDefault();
            var url = $(this).data('url');
            var formData = new FormData(this);
            storeUpdateMaintenanceItem(formData, url)
        });
    });

    // update and store user
    function storeUpdateMaintenanceItem(formData, url) {
        $('.loader-div').show();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = "{{ route('admin.maintenance-items.index') }}";
                    }, 1000);
                } else {
